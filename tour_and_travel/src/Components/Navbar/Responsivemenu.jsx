...
const Responsivemenu = ({ showMenu, setShowMenu }) => {
  return (
    <div className={` ${showMenu ? 'right-0 ' : '-right-[100%]'}  fixed  bottom-0 top-0 x-20 flex h-screen w-[75%] flex-col justify-between  bg-white px-8 pb-6 pt-16 text-black transition-all duration-200 md:hidden rounded-r-xl shadow-md `}>
      <div>
        <button className='border border-black rounded-lg top-4 right-9 ' onClick={() => setShowMenu(false)}>
          <X />
        </button>
        ...
        <nav className='mt-12' >
          <ul className='space-y-4 text-xl text-black flex  flex-col'>
            ...
            <Link to='#'><button onClick={() => setShowMenu(false)} className='bg-red-500 text-white px-4 py-1 rounded-md font-semibold'>
              Book Now
            </button></Link>
      ...
      <div>
        <h1>
          made with ❤️ by <span className='text-red-500'>ayesha </span>
        </h1>
      </div>
    </div>
  )
